...
class LicenseManager:
    ...
    def __init__(self, hass: HomeAssistant) -> None:
        ...
        # Versiunea Home Assistant (import pur, fără I/O)
    ...
    async def async_check_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            _LOGGER.warning(
                "[Vehicule:License] eroare de rețea la verificare status "
                "(încercare #%d) — %s",
            ...
            self._consecutive_failures += 1
            _LOGGER.warning(
                "[Vehicule:License] eroare neașteptată la verificare status "
                "(încercare #%d) — %s",
                self._consecutive_failures,
                err,
            )
    ...
    def is_licensed(self) -> bool:
        ...
        if self._status_token and not self._is_status_cache_valid():
            if self._is_within_grace_period():
                # Perioadă de grație activă — server temporar inaccesibil
                # Licența rămâne validă pe baza token-ului Ed25519 verificat local
                if not self._cache_expiry_warned:
                    ...
                    _LOGGER.warning(
                        "[Vehicule:License] cache expirat — funcționare în perioadă "
                        "de grație (%d ore rămase). Se reîncearcă contactarea serverului.",
                        hours_left,
                    )
                    self._cache_expiry_warned = True
                # NU returnează False — continuă la return True
            else:
                # Perioadă de grație depășită — licență invalidă
                if not self._cache_expiry_warned:
                    _LOGGER.warning(
                        "[Vehicule:License] cache expirat + perioadă de grație depășită "
    ...
    def is_valid(self) -> bool:
        ...
        Scenarii acoperite:
        - Normal: server răspunde, cache valid → (1)
        - Server down temporar: cache expirat, dar grație activă → (2)
        - Backup/restore: storage local gol, server recunoaște fingerprint → (1)
        - Server down prelungit + grație depășită → (3) → probabil False
        """
